...
export function CreateBatchModal({ isOpen, onClose, availableOrders, workers, onCreateBatch }: CreateBatchModalProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-6xl max-h-[90vh] bg-[#1a0d08] border-[#8B4513]/30 text-white overflow-hidden">
        <DialogHeader>
          <DialogTitle className="text-[#d4a574] text-xl">Create New Batch</DialogTitle>
        </DialogHeader>

        <div className="grid grid-cols-3 gap-6 h-[70vh]">
          {/* Available Orders */}
          <div className="space-y-4">
            <h3 className="text-[#d4a574] font-semibold">Available Orders</h3>
                ...
                >
                  <CardContent className="p-3" onClick={() => addOrderToBatch(order)}>
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-[#d4a574] text-sm font-medium">{order.orderNumber}</span>
                      {order.priority !== "normal" && (
                        <Badge className={`text-xs ${order.priority === "urgent" ? "bg-red-600" : "bg-amber-600"}`}>
                          {order.priority}
                        </Badge>
                      )}
                    </div>
                    <div className="text-xs space-y-1">
                      ...
                      <div className="text-white">{order.model}</div>
                      <div className="text-gray-400">{order.woodType}</div>
                      <div className="text-gray-400">Due: {order.dueDate}</div>
                    </div>
                  </CardContent>
                </Card>
              ))}
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-[#d4a574] font-semibold">Batch Orders</h3>
              <Badge className="bg-[#8B4513] text-white">{selectedOrders.length}/12 orders</Badge>
            </div>

            <div className="space-y-2 overflow-y-auto flex-1">
              {selectedOrders.map((order) => (
                <Card key={order.id} className="bg-[#0a0a0a] border-[#8B4513]/20">
            ...
            <div className="space-y-3">
              <div>
                <Label className="text-[#d4a574]">Assign Worker</Label>
                <Select value={assignedWorker} onValueChange={setAssignedWorker}>
                  ...
                  <SelectContent className="bg-[#1a0d08] border-[#8B4513]/30">
                    {workers.map((worker) => (
                      <SelectItem key={worker} value={worker} className="text-white hover:bg-[#8B4513]/20">
                        {worker}
                      </SelectItem>
                    ))}
                  </SelectContent>
              ...
              <div>
                <Label className="text-[#d4a574]">Batch Notes</Label>
                <Textarea
                  value={batchNotes}
          ...
          {/* Efficiency Analysis */}
          <div className="space-y-4">
            <h3 className="text-[#d4a574] font-semibold">Batch Analysis</h3>

            <Card className="bg-[#0a0a0a] border-[#8B4513]/20">
              <CardContent className="p-4">
                <div className="text-center mb-4">
                  <div className="text-3xl font-bold text-[#d4a574] mb-1">{efficiencyScore}%</div>
                ...
                <div className="space-y-2">
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="text-white">{selectedOrders.length} units</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-400">Models:</span>
                    <span className="text-white">{new Set(selectedOrders.map((o) => o.model)).size || 0} types</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-400">Wood Types:</span>
                    <span className="text-white">{new Set(selectedOrders.map((o) => o.woodType)).size || 0} types</span>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="bg-[#0a0a0a] border-[#8B4513]/20">
              <CardContent className="p-4">
                <h4 className="text-[#d4a574] text-sm font-medium mb-3">Recommendations</h4>
                <div className="space-y-2">
