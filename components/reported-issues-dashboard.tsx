    ...
    <div className="min-h-screen bg-gradient-to-br from-[#0a0a0a] to-[#1a0d08] text-white">
        ...
        <div className="flex items-center justify-between px-6 py-4">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <h1 className="text-2xl font-bold text-[#d4a574]">Reported Issues</h1>
                <p className="text-sm text-gray-400">Worker-reported production issues</p>
              </div>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <Badge className="bg-[#8B4513] text-white text-sm px-3 py-1">{filteredIssues.length} Issues</Badge>
          </div>
        </div>
      </header>

      {/* Search and Filters */}
      <div className="px-6 py-4 border-b border-[#8B4513]/30 bg-[#0a0a0a]">
        <div className="flex flex-wrap gap-4 items-center">
          ...
          <div className="flex items-center gap-2">
            <Filter className="h-4 w-4 text-[#d4a574]" />
            <span className="text-sm text-gray-400">Filter by:</span>
          </div>

          <Select value={issueTypeFilter} onValueChange={(value) => setIssueTypeFilter(value as any)}>
            <SelectTrigger className="w-[160px] bg-[#1a0d08] border-[#8B4513]/30 text-white">
              <SelectValue placeholder="Issue Type" />
            </SelectTrigger>
            <SelectContent className="bg-[#1a0d08] border-[#8B4513]/30 text-white">
      ...
      <div className="p-6 space-y-4">
        {filteredIssues.length === 0 ? (
          <div className="text-center py-12">
            ...
            <h3 className="text-xl font-medium text-gray-300">No issues found</h3>
          ...
          filteredIssues.map((issue) => (
            <Card
              key={issue.id}
              className={`bg-[#0a0a0a] border-l-4 ${
                issue.status === "resolved"
                  ...
                  : issue.priority === "critical"
                    ...
                    : issue.priority === "high"
                      ? "border-l-red-600"
                      : "border-l-[#8B4513]"
              } hover:bg-[#1a0d08] transition-colors`}
            >
              <CardContent className="p-4">
                <div className="flex flex-col md:flex-row md:items-start gap-4">
                  {/* Worker Info */}
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-[#1a0d08] flex items-center justify-center overflow-hidden">
                      {issue.workerPhoto ? (
                        <img
                    ...
                    <div>
                      <div className="font-medium text-white">{issue.workerName}</div>
                  ...
                  <div className="flex-1 space-y-3">
                    <div className="flex flex-wrap items-center gap-2">
                      <Badge className={`${getIssueTypeColor(issue.issueType)} text-white`}>
                    ...
                    {issue.status === "resolved" && issue.resolution && (
                      <div className="bg-green-900/20 border border-green-900/30 rounded p-3 mt-2">
                        ...
                        <p className="text-gray-300 text-sm">{issue.resolution}</p>
                  ...
                  <div className="flex md:flex-col gap-2 self-start">
                    <Button
                      ...
                      className="bg-[#1a0d08] hover:bg-[#8B4513]/30 text-white border border-[#8B4513]/30"
                      size="sm"
                    >
                      View Details
                    </Button>
      ...
      {selectedIssue && (
        <Dialog open={!!selectedIssue && !updateStatusDialogOpen} onOpenChange={() => setSelectedIssue(null)}>
          <DialogContent className="bg-[#1a0d08] border-[#8B4513]/30 text-white max-w-3xl">
            <DialogHeader>
              <DialogTitle className="text-[#d4a574] text-2xl flex items-center gap-2">
            ...
            <div className="space-y-6">
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 rounded-full bg-[#0a0a0a] flex items-center justify-center overflow-hidden">
                  {selectedIssue.workerPhoto ? (
                    ...
                    <User className="h-7 w-7 text-[#d4a574]" />
                  )}
                </div>
                <div>
                  <div className="font-medium text-white text-lg">{selectedIssue.workerName}</div>
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <div className="text-white font-medium">{selectedIssue.stage}</div>
                </div>
                <div>
                  <div className="text-gray-400">Model</div>
                  <div className="text-white font-medium">{selectedIssue.model}</div>
                </div>
                <div>
                  <div className="text-gray-400">Batch Number</div>
                  <div className="text-white font-medium">{selectedIssue.batchNumber}</div>
                </div>
                <div>
                  <div className="text-gray-400">Status</div>
                  <Badge className={`${getStatusColor(selectedIssue.status)} text-white mt-1`}>
              ...
              <div>
                <div className="text-gray-400 mb-2">Issue Type</div>
                <Badge className={`${getIssueTypeColor(selectedIssue.issueType)} text-white`}>
              ...
              <div>
                ...
                <div className="bg-[#0a0a0a] border border-[#8B4513]/30 rounded-md p-4 text-white">
              ...
              {selectedIssue.status === "resolved" && selectedIssue.resolution && (
                <div>
                  <div className="text-gray-400 mb-2">Resolution</div>
                  <div className="bg-green-900/20 border border-green-900/30 rounded-md p-4">
                    <p className="text-gray-300">{selectedIssue.resolution}</p>
              ...
              <div className="flex justify-between">
                <Button onClick={() => setSelectedIssue(null)} className="bg-gray-600 hover:bg-gray-700 text-white">
                  Close
                </Button>
      ...
      <Dialog open={updateStatusDialogOpen} onOpenChange={setUpdateStatusDialogOpen}>
        <DialogContent className="bg-[#1a0d08] border-[#8B4513]/30 text-white">
          <DialogHeader>
            <DialogTitle className="text-[#d4a574]">Update Issue Status</DialogTitle>
          </DialogHeader>

          <div className="space-y-4">
            <div className="space-y-2">
              <label className="text-sm font-medium text-[#d4a574]">Status:</label>
              <Select value={newStatus} onValueChange={(value) => setNewStatus(value as any)}>
                ...
                <SelectContent className="bg-[#0a0a0a] border-[#8B4513]/30 text-white">
                  <SelectItem value="open">Open</SelectItem>
                  <SelectItem value="in-progress">In Progress</SelectItem>
                  <SelectItem value="resolved">Resolved</SelectItem>
                </SelectContent>
            ...
            {newStatus === "resolved" && (
              <div className="space-y-2">
                <label className="text-sm font-medium text-[#d4a574]">Resolution:</label>
                <Textarea
                  value={resolution}
                  onChange={(e) => setResolution(e.target.value)}
                  placeholder="Describe how the issue was resolved..."
                  className="bg-[#0a0a0a] border-[#8B4513]/30 text-white placeholder-gray-400 min-h-[100px]"
                />
              </div>
            )}
          </div>
