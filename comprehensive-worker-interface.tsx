...
export default function ComprehensiveWorkerInterface({ onBack }: ComprehensiveWorkerInterfaceProps) {
    ...
    <div className="flex items-center justify-between p-4 bg-[#0a0a0a] border-b border-[#8B4513]/30">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm text-[#d4a574] font-mono">{currentTime.toLocaleTimeString()}</span>
        </div>
      </div>
    </div>
  )
  ...
  const WorkerSelectionPopup = () => (
    <div className="p-6 space-y-6">
      <div className="text-center space-y-4">
        ...
        <div>
          <h1 className="text-3xl font-bold text-[#d4a574] mb-2">Welcome to ZMF Production</h1>
          <p className="text-lg text-gray-300">Select your profile to view your schedule</p>
        </div>
      </div>
          ...
          >
            <CardContent className="p-6 text-center space-y-4">
              <img
                ...
                alt={worker.name}
                className="w-20 h-20 rounded-full mx-auto object-cover border-2 border-[#8B4513]/30"
              />
              <div>
                <h3 className="text-xl font-semibold text-[#d4a574]">{worker.name}</h3>
                <p className="text-sm text-gray-400 capitalize">{worker.currentShift} Shift</p>
              </div>

              <div className="flex flex-wrap gap-1 justify-center">
                {worker.skills.slice(0, 2).map((skill) => (
                  <Badge key={skill} className="bg-[#8B4513] text-white text-xs">
                    {skill}
                  </Badge>
                ))}
                {worker.skills.length > 2 && (
                  <Badge className="bg-gray-600 text-white text-xs">+{worker.skills.length - 2}</Badge>
                )}
              </div>
            </CardContent>
          </Card>
      ...
      <Button onClick={onBack} className="w-full h-14 bg-gray-600 hover:bg-gray-700 text-white">
  ...
  const WeeklyScheduleScreen = () => {
    ...
    return (
      <div className="p-6 space-y-6">
        <div className="text-center space-y-4">
          <div className="w-20 h-20 mx-auto bg-[#8B4513]/20 rounded-full flex items-center justify-center">
            <CalendarDays className="h-10 w-10 text-[#d4a574]" />
          </div>
          <div>
            <h2 className="text-2xl font-bold text-[#d4a574]">Weekly Schedule</h2>
            <p className="text-gray-400">{selectedWorker.name}</p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          {selectedWorker.weeklySchedule.map((day, index) => (
            <Card
              key={index}
              className={`${day.isToday ? "bg-[#8B4513]/20 border-[#d4a574]/50" : "bg-[#1a0d08] border-[#8B4513]/30"}`}
            >
              <CardContent className="p-4">
                <div className="flex items-center justify-between mb-3">
                  <div>
                    <h3 className={`text-lg font-semibold ${day.isToday ? "text-[#d4a574]" : "text-white"}`}>
                      {day.dayName}
                  ...
                  <div className="text-right">
                    <div className={`text-lg font-bold ${day.isToday ? "text-[#d4a574]" : "text-white"}`}>
                      {day.totalEstimatedHours.toFixed(1)}h
                    </div>
                    <div className="text-sm text-gray-400">{day.assignments.length} tasks</div>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  {day.assignments.map((assignment, assignmentIndex) => (
                    <div
                      key={assignmentIndex}
                      className="flex items-center justify-between p-2 bg-[#0a0a0a]/50 rounded border border-[#8B4513]/20"
                    >
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                          <span className="text-white font-medium">{assignment.model}</span>
        ...
        <Button
          ...
          className="w-full h-14 bg-gray-600 hover:bg-gray-700 text-white"
        >
          <ArrowLeft className="h-5 w-5 mr-2" />
          Change Worker
        </Button>
  ...
  const ActiveWorkScreen = () => (
    <div className="p-6 space-y-6">
      <div className="text-center space-y-4">
        <div className="text-6xl font-bold text-[#d4a574] font-mono">{formatTime(elapsedTime)}</div>
        <div className="text-lg text-gray-300">Target: {currentAssignment?.estimatedTime}</div>
      </div>
          ...
          <CardContent className="p-6 text-center space-y-4">
            ...
            <div className="grid grid-cols-3 gap-4 text-sm">
              <div>
                ...
                <div className="text-white font-medium">{currentAssignment.batchNumber}</div>
              </div>
              <div>
              ...
              <div>
                <div className="text-gray-400">Wood</div>
                <div className="text-white font-medium">{currentAssignment.woodType}</div>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <div className="flex gap-4">
        {isTimerRunning ? (
          <Button
            ...
            className="flex-1 h-16 bg-amber-600 hover:bg-amber-700 text-white text-lg"
          >
          ...
          <Button
            ...
            className="flex-1 h-16 bg-green-600 hover:bg-green-700 text-white text-lg"
          >
            <Play className="h-6 w-6 mr-2" />
            Resume Work
          </Button>
        ...
        <Button
          ...
          className="flex-1 h-16 bg-[#8B4513] hover:bg-[#8B4513]/80 text-white text-lg"
        >
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <Button onClick={() => setShowIssueDialog(true)} className="h-14 bg-red-600 hover:bg-red-700 text-white">
    ...
    return (
      <div className="p-6 space-y-6">
        <div className="text-center space-y-2">
          <h2 className="text-2xl font-bold text-[#d4a574]">Quality Control</h2>
          ...
          <CardContent className="p-6 space-y-6">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-2">
                  <h3 className="text-lg font-medium text-white">{currentCheck.description}</h3>
                  {currentCheck.criticalPoint && <Badge className="bg-red-600 text-white text-xs">CRITICAL</Badge>}
                </div>
                <p className="text-sm text-gray-400 capitalize">{currentCheck.category} inspection</p>
              </div>
            </div>
              ...
              <div className="space-y-3">
                <Button
                  ...
                  className={`w-full h-16 ${
                    currentCheck.photoTaken ? "bg-[#8B4513] hover:bg-[#8B4513]/80" : "bg-blue-600 hover:bg-blue-700"
                  } text-white text-lg`}
                >
            ...
            <div className="space-y-3">
              <Textarea
                ...
                className="bg-[#0a0a0a] border-[#8B4513]/30 text-white placeholder-gray-400 min-h-[80px]"
              />
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <Button
                ...
                className="h-16 bg-green-600 hover:bg-green-700 text-white text-lg disabled:opacity-50"
              >
                <CheckCircle className="h-6 w-6 mr-2" />
                Pass
              </Button>
        ...
        <div className="flex gap-4">
          ...
          {currentCheckIndex > 0 && (
            <Button
              ...
              className="h-14 bg-[#8B4513] hover:bg-[#8B4513]/80 text-white px-6"
            >
              Previous
            </Button>
          )}
    ...
    return (
      <div className="p-6 space-y-6">
        <div className="text-center space-y-4">
          ...
          <div>
            <h2 className="text-2xl font-bold text-[#d4a574]">Task Complete!</h2>
            <p className="text-gray-400">Excellent work on this stage</p>
          </div>
        </div>
        ...
        <Card className="bg-[#1a0d08] border-[#8B4513]/30">
          <CardContent className="p-6 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4 text-center">
              <div>
                <div className="text-2xl font-bold text-[#d4a574]">{formatTime(elapsedTime)}</div>
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                <span className="text-gray-400">Stage: {currentAssignment?.stage}</span>
                <span className="text-green-400">✓ Complete</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-400">Batch: {currentAssignment?.batchNumber}</span>
        ...
        <div className="space-y-4">
          <Button
            ...
            className="w-full h-16 bg-[#8B4513] hover:bg-[#8B4513]/80 text-white text-lg font-semibold"
          >
  ...
  const BreakMenuScreen = () => (
    <div className="p-6 space-y-6">
      <div className="text-center space-y-4">
        ...
        <div>
          <h2 className="text-2xl font-bold text-[#d4a574]">Break Time</h2>
          <p className="text-gray-400">Take care of yourself</p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        <Button className="w-full h-16 bg-amber-600 hover:bg-amber-700 text-white text-lg">
          <Clock className="h-6 w-6 mr-2" />
          End of Shift
        </Button>
      </div>
  ...
  // Main render
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#0a0a0a] to-[#1a0d08] text-white max-w-md mx-auto">
      <StatusBar />
      ...
      <Dialog open={showIssueDialog} onOpenChange={setShowIssueDialog}>
        <DialogContent className="bg-[#1a0d08] border-[#8B4513]/30 text-white">
          <DialogHeader>
            <DialogTitle className="text-[#d4a574]">Report Issue</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <Textarea
            ...
            <div className="grid grid-cols-2 gap-2">
              <Button className="bg-red-600 hover:bg-red-700 text-white">Quality Issue</Button>
              <Button className="bg-amber-600 hover:bg-amber-700 text-white">Equipment Issue</Button>
              <Button className="bg-blue-600 hover:bg-blue-700 text-white">Material Issue</Button>
              <Button className="bg-purple-600 hover:bg-purple-700 text-white">Other Issue</Button>
            </div>
          </div>
          <DialogFooter>
            <Button
            ...
            <Button onClick={() => setShowIssueDialog(false)} className="bg-[#8B4513] hover:bg-[#8B4513]/80 text-white">
              Submit Report
            </Button>
          </DialogFooter>
        </DialogContent>
